...
class Swiftris {
    ...
    func newShape() -> (fallingShape:Shape?, nextShape:Shape?) {
        ...
        if detectIllegalPlacement() {
            ...
            nextShape!.moveTo(StartingColumn, row: PreviewRow)
            endGame()
            return (nil, nil)
        }
    ...
    func removeCompletedLines() -> (linesRemoved: Array<Array<Block>>, fallenBlocks: Array<Array<Block>>) {
        ...
        for column in 0..<NumColumns {
            var fallenBocksArray = Array<Block>()
            ...
            for var row = removedLines[0][0].row - 1; row > 0; row-- {
                if let block = blockArray[column, row] {
                    ...
                    fallenBocksArray.append(block)
                }
            }
            if fallenBocksArray.count > 0 {
                fallenBlocks.append(fallenBocksArray)
            }
        }
    ...
    func removeAllBlocks() -> Array<Array<Block>> {
        ...
        for row in 0..<NumRows {
            var rowOfBlocks = Array<Block>()
            for colum in 0..<NumColumns {
                if let block = blockArray[colum, row] {
                    rowOfBlocks.append(block)
                    blockArray[colum, row] = nil
                }
            }
